        ...
        public void runAndroidCommand()
        {
            ...
            if (!String.IsNullOrEmpty(ResourceApkPath))
            {
                ...
                foreach (string itemName in Config.ResourceFileList)
                {
                    ...
                    {
                        resourceApp.DirectoryCopy(resourceApp.getTempPath(pathName), androidApp.getTempPath(pathName), needEncry);
                    }
                    else if (File.Exists(resourceApp.getTempPath(pathName)))
                    {
                        resourceApp.FileCopy(resourceApp.getTempPath(pathName), androidApp.getTempPath(pathName), needEncry);
                    }
                }
            ...
            foreach (string strPath in EncryptOverwrite)
            {
                ...
                {
                    androidApp.DirectoryCopy(strPath, androidApp.getAssetPath(""), true);
