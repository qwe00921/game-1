...
namespace SceneEditor.Tools
{
    public static class EexFile
    {
        ...
        public static ScriptRoot loadFile(String path, int group)
        {
            switch (getFileType(path))
            {
                case EexFileType.TYPE_EEX:
                    return loadEexFile(path, group);
                case EexFileType.TYPE_EEX_NEW:
        ...
        public static ScriptRoot loadEexFile(String path, int group)
        {
            using (FileStream fs = new FileStream(path, FileMode.Open))
            {
                ...
                root.loadReader(new EexBinaryReader(buffer), group);
                return root;
            }
        }
        ...
        public static void saveFile(ScriptRoot root, String path, int group)
        {
            switch (getFileType(path))
            {
                case EexFileType.TYPE_EEX:
                    saveEexFile(root, path, group);
                    break;
                case EexFileType.TYPE_EEX_NEW:
