...
{	
	...
	len=original->length();
	alloc(len+1);
	memcpy(m_pchData, original->getNativeCharPtr(), (len+1)*sizeof(char));
}
...
{
	char* ret= strstr(m_pchData+fromIndex, str->getNativeCharPtr());
	return (ret==0)? -1 : ret-m_pchData;
}
...
char* DString::getNativeCharPtr()
{
	return m_pObject->getNativeCharPtr(); 
}
