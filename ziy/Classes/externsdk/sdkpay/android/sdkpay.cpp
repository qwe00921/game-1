...
int payStartSDKpay(const char* trGameId, const char* outTradeNo, const char* subject, const char* body, const char* totalFee)
{
	...
	bool isHave = JniHelper::getStaticMethodInfo(minfo, "com/bantu/trgame/TRGame", "startSDKpay", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)I");
	if(!isHave)
	{
		CCLog("jni:startSDKpay not found");
		return 0;
	}
...
extern "C"
{
	JNIEXPORT void Java_com_bantu_trgame_TRGame_nativeSDKpayResult(JNIEnv*  env, jobject thiz, jstring tradeNo, jint resultCode, jstring resultMessage)
	{
		const char* strTradeNo = env->GetStringUTFChars(tradeNo, 0);
		const char* strResultMessage = env->GetStringUTFChars(resultMessage, 0);

		StatConfig::onPayResult(StatConfig::PAY_TYPE_QDSDK, strTradeNo, resultCode, strResultMessage);
	}
/*
	do in sdk
	
	JNIEXPORT void Java_com_bantu_trgame_TRGame_nativeSDKHttpRequest(JNIEnv*  env, jobject thiz, jstring appid, jstring mch_id, jstring out_trade_no, jstring nonce_str)
	{
		...
		StatConfig::startSDKHttpRequest(strAppid, strMch_id, strOut_trade_no, strNonce_str, g_strSubject, g_strBody);
	}
	*/
}