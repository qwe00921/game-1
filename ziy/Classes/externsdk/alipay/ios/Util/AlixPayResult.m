	...
	if (self = [super init]) {
		...
		self.statusMessage = @"未知错误。";
            ...
            do {
                ...
                //NSDictionary * jsonMemo = [dictionary objectForKey:@"memo"];
                
                self.statusCode = [[dictionary objectForKey:@"resultStatus"] intValue];
                self.statusMessage = [dictionary objectForKey:@"memo"];
                if (self.statusCode != 9000) {
                    break;
                }
                
                NSString *resultString = [dictionary objectForKey:@"result"];
