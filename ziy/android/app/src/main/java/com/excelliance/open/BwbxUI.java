...
public class BwbxUI {
    ...
    private BroadcastReceiver bwbxbr = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if ((!action.equals(getPackageName()+BwbxUtilHelper.getString(mContext, "ACTION_QUERY_USER", "")) || intent.getIntExtra("type", 0) != BwbxUtilHelper.getInt(mContext, "ACTION_TYPE_PRELOAD_DIALOG", 0)) && (!BaseUtil.isForeground(mContext) || (topActivity != null && (topActivity.contains("NextChapter") || topActivity.equals("com.excelliance.open.PromptActivity"))))) {
    ...
    private void showDownloadFullRes(long size, final boolean show) {
        ...
        if(!downloadFullRes.isShowing() && show) {
            downloadFullRes.setMessage(String.format(lebian_choose_by_user_bwbx,((float)(size))/(1024.0f * 1024.0f)));
